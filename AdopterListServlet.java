...
public class AdopterListServlet extends HttpServlet {
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      String query = "SELECT * FROM Member WHERE Member.IsAdopter = ?";
      ...
      while (rset.next ()){
        ...
        for(int i=1; i<9; i++){
          if(i<8){
            // add commas between column items except the last one
            currAdopt += rset.getString (i) + ", ";
          }
          else{
            currAdopt += rset.getString (i);
          }
        }
        out.println (" <li> " + currAdopt + "</li>");
      }
